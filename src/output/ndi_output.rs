//! NDI output sender.
//!
//! Sends BGRA video frames as an NDI stream on a background thread.
...
/// NDI output sender — owns a background thread that drains a bounded channel.
...
impl NdiOutputSender {
    pub fn new(name: impl Into<String>, width: u32, height: u32, include_alpha: bool) -> anyhow::Result<Self> {
        ...
        // Leak the join handle — the thread will exit when `running` is set to false
        // and the channel disconnects. This matches the template pattern.
    ...
    /// Submit a frame for sending (non-blocking, drops if channel full).
